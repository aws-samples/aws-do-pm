...
def sample_partial(problem, Nss, idxNss, deltaNss, calc_second_order=True, seed=None, skip_values=1000):
    """Generates model inputs using Saltelli's extension of the Sobol sequence.
    ...
    # Create base sequence - could be any type of sampling
    # base_sequence = sobol_sequence.sample(N + skip_values, 2 * D)
    base_sequence = sobol_sequence.sample(Nss + skip_values, 2 * D)

    if calc_second_order:
        # saltelli_sequence = np.zeros([(2 * Dg + 2) * N, D])
        saltelli_sequence = np.zeros([(2 * Dg + 2) * deltaNss, D])
    else:
        # saltelli_sequence = np.zeros([(Dg + 2) * N, D])
        saltelli_sequence = np.zeros([(Dg + 2) * deltaNss, D])
    index = 0

    #for i in range(skip_values, N + skip_values):
    for i in range(skip_values + idxNss, deltaNss + skip_values + idxNss):
