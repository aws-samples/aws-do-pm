...
def app(pm_root_path):
    ...
    model_id_dict = data_model_id_dict['model']
    def format_func(option):
        cur_dict = model_id_dict[option]
    ...
    if model_id_dict:
        sel_id = st.selectbox('Select Model', options=list(model_id_dict.keys()), format_func=format_func)

        # sel_id = st.selectbox('Select ID', data_model_id_dict['model'])
        print('Selected ID: %s'%(sel_id))

        # Read the full path of collection + selected_id
        id_path_rel = 'model/%s'%(sel_id)
