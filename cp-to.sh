...
function usage {
	echo ""
	echo "Usage: $0 <container_name> <instance_num> <src_path> <dest_path> - copies local src_path to container:<dest_path>"
	echo ""
}
...
case "${TO}" in
		...
		if [ "${COMPOSE_CONTEXT_TYPE}" == "moby" ]; then
			...
			CMD="docker cp ${SRC_PATH} ${COMPOSE_PROJECT_NAME}_${SERVICE_NAME}_${CONTAINER_INDEX}:${DEST_PATH} "
	...
	"swarm")
		...
		if [ "${CONTAINER_ID}" == "" ]; then
			...
			CMD="docker cp ${SRC_PATH} ${CONTAINER_ID}:${DEST_PATH}"
		fi
		;;
	"ecs")
		echo ""
	...
	"kubernetes")
		...
		CMD="unset DEBUG; ${KUBECTL} -n ${NAMESPACE} cp ${SRC_PATH} $( ${KUBECTL} -n ${NAMESPACE} get pod | grep ${POD_GROUP} | head -n ${CONTAINER_INDEX} | tail -n 1 | cut -d ' ' -f 1 ):${DEST_PATH} "
		;;
	"lambda")
		echo ""
		echo "Copy operation is not supported for target orchestrator $TO"
